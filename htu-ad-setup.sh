...
surface_entrypoint_global() { # surface_entrypoint_global <root> <tool> <path/inside/repo>
    # Like surface_entrypoint, but for pentest-tools/OT-tools: makes the command
    # reachable from any directory (symlink in /usr/local/bin), not just from
    # inside the cloned repo. Uses a small cd-then-exec wrapper rather than a
    # bare symlink, so a script with sibling imports (e.g. plcscan.py importing
    # modbus.py) still finds them regardless of the caller's cwd or PATH lookup.
...
# ================ 9. OT-TOOLS (Modbus/ICS/SCADA-specific) ====================
...
# system-wide, same treatment as every other OT-tools entry. ---
