...
# apt-pkg|binary|pipx-spec|friendly-name
PACKAGES=(
...
if [[ -x "${LIGOLO_PROXY}/proxy" ]]; then
    ...
    if run curl -fsSL -o "${TMP}/proxy.tar.gz" \
        "${LIGOLO_BASE}/ligolo-ng_proxy_${LIGOLO_VER}_linux_${LIGOLO_ARCH}.tar.gz"; then
        rm -rf "${TMP}/px" && mkdir -p "${TMP}/px"
        run tar -xzf "${TMP}/proxy.tar.gz" -C "${TMP}/px"
        if [[ -f "${TMP}/px/proxy" ]]; then
            mv "${TMP}/px/proxy" "${LIGOLO_PROXY}/proxy"
            chmod +x "${LIGOLO_PROXY}/proxy"
            cp -f "${TMP}/px/"{LICENSE,README.md} "${LIGOLO_PROXY}/" 2>/dev/null
...
if [[ -x "${LIGOLO_AGENTS}/linux-agent" ]]; then
    ...
    if run curl -fsSL -o "${TMP}/lagent.tar.gz" \
        "${LIGOLO_BASE}/ligolo-ng_agent_${LIGOLO_VER}_linux_${LIGOLO_ARCH}.tar.gz"; then
        rm -rf "${TMP}/la" && mkdir -p "${TMP}/la"
        run tar -xzf "${TMP}/lagent.tar.gz" -C "${TMP}/la"
        if [[ -f "${TMP}/la/agent" ]]; then
            mv "${TMP}/la/agent" "${LIGOLO_AGENTS}/linux-agent"
...
# --- windows agent ---
if [[ -f "${LIGOLO_AGENTS}/windows-agent.exe" ]]; then
    skip "ligolo windows-agent already present"
else
    info "downloading ligolo-ng windows agent..."
    if run curl -fsSL -o "${TMP}/wagent.zip" \
        "${LIGOLO_BASE}/ligolo-ng_agent_${LIGOLO_VER}_windows_amd64.zip"; then
        rm -rf "${TMP}/wa" && mkdir -p "${TMP}/wa"
        run unzip -o -q "${TMP}/wagent.zip" -d "${TMP}/wa"
        if [[ -f "${TMP}/wa/agent.exe" ]]; then
            mv "${TMP}/wa/agent.exe" "${LIGOLO_AGENTS}/windows-agent.exe"
...
check_dir "${TOOLS}/GhostSPN"                                   "GhostSPN"
...
check_dir "${TOOLS}/ldapsearch/ldapsearch.py"                   "ldapsearch.py"
check_dir "${TOOLS}/powerview.py"                               "powerview.py"
