...
for entry in "${GIT_TOOLS[@]}"; do
    ...
    clone_tool "$name" "$url"
    # PassTheCert keeps its python client one level down, next to the C# port;
    # surface it at the tool root so the path is just PassTheCert/passthecert.py
    if [[ "$name" == "PassTheCert" ]]; then
        if [[ -f "${TOOLS}/PassTheCert/Python/passthecert.py" ]]; then
            chmod +x "${TOOLS}/PassTheCert/Python/passthecert.py"
            ln -sf "Python/passthecert.py" "${TOOLS}/PassTheCert/passthecert.py"
            run pip3 install --break-system-packages --no-input -q ldap3 pyasn1 pycryptodome \
                || note "${YELLOW}some passthecert.py deps failed (see log)${NC}"
            note "passthecert.py linked at ${TOOLS/#$HOME/~}/PassTheCert/passthecert.py"
        else
            fail "passthecert.py (not found in the cloned PassTheCert repository)"
        fi
    fi
done
